jest.mock(
	'./../../../lib/handlers/subServiceRequested/subServiceRequestToApprovers'
);
...
describe('when registering sub-service request handler', () => {
	...
	beforeAll(() => {
		subServiceRequested = require('../../../lib/handlers/subServiceRequested/subServiceRequestToApprovers');
		subServiceRequested.getHandler = jest.fn().mockReturnValue({
			type: 'sub_service_request_to_approvers',
			processor,
		});

		register = require('../../../lib/handlers/subServiceRequested').register;
	});

	it('then it should register the sub_service_request_to_approvers handler', async () => {
		const actual = await register(config, logger);
		const handler = actual.find((x) => x.type === 'sub_service_request_to_approvers');
