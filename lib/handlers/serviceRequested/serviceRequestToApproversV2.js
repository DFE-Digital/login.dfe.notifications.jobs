...
const process = async (config, logger, data) => {
  try {
    ...
    const approverIds = approversForOrg.map((approver) => approver.id);
    const activeApproverIds = await directories.getAllActiveUsersFromList(approverIds);
    const activeApprovers = approversForOrg.filter((entity) => activeApproverIds.find(entity.id));

    if (activeApprovers.length > 0) {
      const approvers = await directoriesClient.getUsersByIds(activeApprovers)
