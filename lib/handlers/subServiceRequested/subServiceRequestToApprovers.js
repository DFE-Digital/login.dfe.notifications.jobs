...
const execute = async (config, logger, data) => {
  try {
    ...
    let subject = 'A user has requested access to a sub-service';
    ...
    logger.error(`Failed to process and send the email for type sub_service_request_to_approvers - ${JSON.stringify(e)}`);
    throw new Error(`Failed to process and send the email for type sub_service_request_to_approvers - ${JSON.stringify(e)}`);
  }
};
